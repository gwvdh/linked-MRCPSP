...
def continuous_model(n, T, M, R, E, VP, p, L, r, ES=None, silent=True, obj="makespan"):
    """
    ...
    if obj == "makespan":
        ...
        model.setObjective(gp.quicksum(S[i] + gp.quicksum(p[i][m] * x[i, m] for m in range(M)) for i in range(n)), GRB.MINIMIZE)
